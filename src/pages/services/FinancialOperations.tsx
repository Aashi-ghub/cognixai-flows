import { motion } from "framer-motion";
import { Navbar } from "@/components/landing/Navbar";
import { Footer } from "@/components/landing/Footer";
import { ArrowRight, DollarSign, RefreshCcw, TrendingUp } from "lucide-react";
import { useContactPopup } from "@/contexts/ContactPopupContext";
import { ServiceSchema } from "@/components/SeoSchemas";

const services = [
  {
    icon: DollarSign,
    title: "Autonomous Revenue Recovery",
    description: "AI-powered systems that identify and recover lost revenue opportunities automatically."
  },
  {
    icon: RefreshCcw,
    title: "AP/AR Reconciliation Automation",
    description: "Streamline accounts payable and receivable with intelligent matching and exception handling."
  },
  {
    icon: TrendingUp,
    title: "Predictive Cash Flow Modeling",
    description: "Forecast cash positions with AI-driven insights for better financial planning."
  }
];

const FinancialOperations = () => {
  const { openPopup } = useContactPopup();

  return (
    <div className="min-h-screen bg-background">
      <ServiceSchema
        name="AI Agents for Financial Operations"
        description="Automate 70–85% of financial ops workflows including reconciliation, reporting, invoice processing, and compliance checks with AI agents."
      />
      <Navbar />
      
      {/* Hero Section */}
      <section className="pt-40 pb-20 bg-gradient-to-b from-[#f5f3ee] to-background">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6 }}
            className="max-w-4xl mx-auto text-center"
          >
            <span className="inline-block px-4 py-1.5 mb-6 text-xs font-medium tracking-wider uppercase bg-[#4a7c59]/10 text-[#4a7c59] rounded-full">
              Service Category
            </span>
            <h1 
              className="text-4xl md:text-5xl lg:text-6xl mb-6 text-foreground"
              style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}
            >
              Financial Operations & Audit Automation
            </h1>
          </motion.div>
        </div>
      </section>

      {/* Why This Matters */}
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="max-w-3xl mx-auto"
          >
            <h2 
              className="text-2xl md:text-3xl mb-6 text-foreground"
              style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}
            >
              Why This Matters
            </h2>
            <p className="text-lg text-muted-foreground leading-relaxed">
              Finance is the heartbeat of the enterprise. By injecting intelligent automation here, 
              we create self-funding initiatives that pay for further innovation. This cluster focuses 
              on precision, reconciliation, and cash preservation.
            </p>
          </motion.div>
        </div>
      </section>

      {/* Green Banner */}
      <section className="py-8 md:py-12">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="bg-[#5c6650] rounded-2xl py-10 md:py-14 px-6 flex flex-col items-center justify-center"
          >
            <h2
              className="text-2xl md:text-3xl lg:text-4xl text-white text-center max-w-3xl"
              style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}
            >
              Included Services
            </h2>
          </motion.div>
        </div>
      </section>

      {/* Services Grid */}
      <section className="py-16 md:py-24 bg-[#f5f3ee]">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <motion.div
                key={service.title}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
                className="bg-background rounded-xl p-8 border border-border hover:shadow-lg transition-shadow"
              >
                <div className="w-14 h-14 bg-[#4a7c59]/10 rounded-lg flex items-center justify-center mb-6">
                  <service.icon className="w-7 h-7 text-[#4a7c59]" />
                </div>
                <h3 
                  className="text-xl mb-3 text-foreground"
                  style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}
                >
                  {service.title}
                </h3>
                <p className="text-muted-foreground">
                  {service.description}
                </p>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-4">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.6 }}
            className="max-w-2xl mx-auto text-center"
          >
            <h2 
              className="text-3xl md:text-4xl mb-6 text-foreground"
              style={{ fontFamily: "'Instrument Serif', Georgia, serif" }}
            >
              Ready to transform your financial operations?
            </h2>
            <p className="text-lg text-muted-foreground mb-8">
              Let's discuss how our automation solutions can help your organization.
            </p>
            <button
              onClick={openPopup}
              className="inline-flex items-center gap-2 px-8 py-4 bg-foreground text-background rounded-lg font-medium hover:bg-foreground/90 transition-colors"
            >
              Get Started
              <ArrowRight className="w-5 h-5" />
            </button>
          </motion.div>
        </div>
      </section>

      <Footer />
    </div>
  );
};

export default FinancialOperations;
